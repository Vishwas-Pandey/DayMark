...
import { Download, RefreshCw, Activity, Target, Zap, LayoutDashboard, HeartPulse } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
      <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-interactive-primary/10 border border-interactive-primary/20 rounded-xl shadow-sm">
            ...
            <span className="text-sm font-semibold text-text-heading">{getScore(data?.scores?.productivity)} <span className="text-text-muted font-medium">Productivity</span></span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 border border-purple-200 rounded-xl shadow-sm">
            <Zap size={16} className="text-purple-500" />
            <span className="text-sm font-semibold text-text-heading">{getScore(data?.scores?.focus)} <span className="text-text-muted font-medium">Focus</span></span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-xl shadow-sm">
            <LayoutDashboard size={16} className="text-blue-500" />
            <span className="text-sm font-semibold text-text-heading">{getScore(data?.scores?.consistency)} <span className="text-text-muted font-medium">Consistency</span></span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-200 rounded-xl shadow-sm">
            <Target size={16} className="text-green-500" />
            <span className="text-sm font-semibold text-text-heading">{getScore(data?.scores?.completion)} <span className="text-text-muted font-medium">Completion</span></span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-pink-50 border border-pink-200 rounded-xl shadow-sm">
            ...
            <span className="text-sm font-semibold text-text-heading">{getScore(data?.scores?.wellbeing)} <span className="text-text-muted font-medium">Wellbeing</span></span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <button 
          className="flex items-center gap-2 px-4 py-2 bg-surface-primary text-text-heading border border-border-default font-semibold rounded-xl hover:bg-surface-secondary transition-all shadow-sm"
        >
          <Download size={18} />
